...
static void gpioInit(void) {
	...
	// Reflexkoppler-LED
	gpio_mode_setup(REFLEX_LED_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE,
	REFLEX_LED_PIN);

	// Reflexkoppler-Fototransistor
	gpio_mode_setup(REFLEX_INPUT_PORT, GPIO_MODE_INPUT, GPIO_PUPD_PULLUP,
	REFLEX_INPUT_PIN);
	...
	for (int i = 0; i < DMA_BANK_SIZE;) {
		...
		if (led_cur < LED_COUNT) {
			...
			for (int j = 0; j < 24; j++) { // schreibe untere 24 Bit jedes 32-Bit Wertes
				dma_data_bank[i++] =
						(v & 0x800000) ? (uint8_t) WS1 : (uint8_t) WS0; // Timerwert für 1 oder 0 schreiben
...
static void generateDmaStepperData(uint16_t* dma_data, uint16_t offset,
		uint16_t letzterSchritt) {
...
static void populate_stepper_data(uint16_t *dma_data_bank) {
	...
	for (int i = 0; i < DMA_STEPPER_BANK_SIZE / 2; i++) {
		...
		if (aktV > 0) {
			restschritte = (deltaV * deltaV) / (2 * aktA)
					+ (sollV * deltaV) / aktA;
		}
...
int main(void) {
	...
	while (1) {
		...
		uartSendBlkDat(USART1, "wechsle LED-Farbe...\r\n");

		led_data[0] = 0x00FF0000;
		delay();
		led_data[0] = 0x0000FF00;
		delay();
		led_data[0] = 0x000000FF;
		delay();
		...
		if (gpio_get(GPIOA, GPIO0) == 1) {
			...
			led_data[0] = (led_data[0] & 0xFFFF0000)
					| ((led_data[0] & 0xFF00) + 0x1000);
